...
CREATE TABLE p_characters (
    ...
    name VARCHAR(16) NOT NULL UNIQUE CHECK (name ~ '^[A-Za-z]{3,16}$'),
    hair_color VARCHAR(7) NOT NULL CHECK (hair_color ~ '^#[0-9A-Fa-f]{6}$'),
    -- ここで hp hp_max の順序を逆にしても成立する
    ...
    created_on DATE NOT NULL CHECK (created_on BETWEEN '2025-12-1' AND CURRENT_DATE),
